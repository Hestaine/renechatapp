  ...
  return (
    <div className="container mx-auto">
      ...
      <form className="flex flex-col items-center gap-[0.47rem]" action="" method="">
        ...
        <input type="email" name="email" className="h-9 rounded text-white outline-none px-3 w-2/4 shadow-sm shadow-blue-400" value={FormState.email} onChange={(e) => { setFormState({ ...FormState, [e.target.name]: e.target.value }) }} />
        <label htmlFor="username" className="text-left">Username:</label>
        <input type="text" name="username" className="h-9 rounded text-white outline-none px-3 w-2/4 shadow-sm shadow-blue-400" value={FormState.username} onChange={(e) => { setFormState({ ...FormState, [e.target.name]: e.target.value }) }} />
        <label htmlFor="password" className="text-left">Password:</label>
        <input type="password" name="password" id="password" className="h-9 rounded text-white outline-none px-3 w-2/4 shadow-sm shadow-blue-400" value={FormState.password} onChange={(e) => { setFormState({ ...FormState, [e.target.name]: e.target.value }) }} />
        <label htmlFor="password" className="text-left">Confirm Password:</label>
        <input type="password" name="secpassword" id="secpassword" className="h-9 rounded text-white outline-none px-3 w-2/4 shadow-sm shadow-blue-400" value={secpassword} onChange={(e) => { setSecPassword(e.target.value) }} />
        <input type="submit" className="bg-blue-500 text-white rounded h-10 w-2/4 m-3 mt-[1r.5em] shadow-md shadow-slate-400" value="Sign Up" onClick={(e) => handleSubmit(e)} />
        <p className="m-2">Have an account? <Link to="/login" className="hover:text-blue-400">Log In</Link></p>
